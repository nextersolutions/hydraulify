...
// The viewer's export adds its own full-size background rect filled with its
// theme colour, which is archify's dark navy by default. Paint it as paper.
const EXPORT_BACKGROUND = '  .hy-root > rect[width="100%"] { fill: var(--paper); }\n';
...
export function renderHtmlDocument(model, analysis, { template = loadViewerTemplate() } = {}) {
  ...
  // The stylesheet and paper travel inside the SVG, not only in the page head:
  // the viewer's exports serialize this element on its own and keep only the
  // page rules whose selectors look like archify's, so without them every
  // exported image came out black-filled and unstroked.
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" class="hy-root" role="img"`
    ...
    + `<style>${STYLESHEET}${EXPORT_BACKGROUND}</style>`
