...
This file is the Codex-facing copy of the hydraulify instructions. The block
between the SHARED markers is generated from `docs/shared-instructions.md` and
is byte-identical to the one in `SKILL.md`; edit the source file, then run
...
There is no structured picker here, so ask in prose, in one message, using a
numbered list per open choice:

```
Two things change how this circuit behaves and the description does not settle them:

1. Directional valve configuration
   a) 4/3 - three positions, lets the actuator be stopped mid-stroke
   b) 4/2 - two positions, the actuator is always driven one way or the other

2. Centre condition (only if 4/3)
   a) closed  - all ports blocked; holds the actuator, pump deadheads over relief
   b) tandem  - P to T open; unloads the pump at rest, still holds the actuator
   c) open    - all ports joined; actuator free, pump unloaded
   d) float   - A, B and T joined; actuator free, pump deadheads

Reply with the letters, or say "use the defaults" and I will record each as an
assumption on the drawing.
```

Take a free-text answer literally where it is unambiguous ("tandem" picks 2b).
Where it is not ("the normal one"), say which reading you took and record it as
an assumption rather than guessing silently.

If nobody can answer -- a scripted or batch run -- do not stop: apply the
documented defaults, record each as an assumption, and say plainly in your
response that those choices were made without confirmation.
